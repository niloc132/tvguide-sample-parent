...
public class ModelProviderImpl extends HybridServiceServlet implements ModelProvider {
    public static final EntityManagerFactory EMF = Persistence.createEntityManagerFactory("laptopsanytime-ops");
...
*/
       ...
       // embedded-land
        ...
        try {
            try {
                ...
                entityClass = Class.forName("laptopsanytime.model." + classname);
            }
            o = em.find(entityClass, id);
        } catch (ClassNotFoundException e) {
            Logger.getAnonymousLogger().warning(Arrays.toString(e.getStackTrace())); //To change body of catch statement use File | Settings | File Templates.
            return null;
        }
        ...
        for (Method method : entityClass.getDeclaredMethods()) {
            try {
                ...
                if (mName.startsWith("get")) {
                    final String baseFieldName = new StringBuilder().append(Character.toLowerCase(mName.charAt(3))).append(mName.substring(4)).toString();
                        ...
                        try {
                            ...
                            if (genericFieldType instanceof ParameterizedType) {
                                ...
                                for (Type fieldArgType : fieldArgTypes) {
                                    ...
                                    System.err.println("@" + field.getName() + "@" + fieldArgClass.getName());
                                    aggregates.put(field.getName(), fieldArgClass);
                                }
                            }
        ...
        if (o != null) {
                ...
                if (map.containsKey(k)) {
                    try {
                        ...
                        if (!(value instanceof Iterable))
                            r.put(String.valueOf(k), String.valueOf(value == null ? "" : value));
                    } catch (Exception e) {
                        Logger.getAnonymousLogger().warning(Arrays.toString(e.getStackTrace())); //To change body of catch statement use File | Settings | File Templates.
                    }
                }
            ...
            for (Map.Entry<String, Class> entry : aggregates.entrySet()) {
                ...
                for (Object o1 : value) {
                    try {
                        final Method declaredMethod = entity.getDeclaredMethod("getId");
                ...
                r.put("@" + k + "@" + entity.getSimpleName(), Arrays.toString(s.toArray()));
            }
            for (Map.Entry<String, Class> entry : linkMap.entrySet()) {
                ...
                try {
                    ...
                    final String k = entry.getKey();
                    r.put("+" + k + "+" + entity.getSimpleName(), String.valueOf(declaredMethod.invoke(map.get(k))));

                } catch (NoSuchMethodException e1) {
                    Logger.getAnonymousLogger().warning(Arrays.toString(e1.getStackTrace())); //To change body of catch statement use File | Settings | File Templates.
    ...
    public boolean merge
            (String
                    tabTitle, String
                    id, Map<String, String> updateMap, String
                    sid) {
        final PartnerAccess partnerAccess;
        ValidationMethod validationMethod = new ValidationMethod(sid).invoke();
        PartnerSession session = validationMethod.getSession();
